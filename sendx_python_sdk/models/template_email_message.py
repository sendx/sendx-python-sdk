...
class TemplateEmailMessage(BaseModel):
    """
    TemplateEmailMessage
    """ # noqa: E501
    var_from: XFrom = Field(alias="from")
    to: Annotated[List[XTo], Field(min_length=1)]
    reply_to: Optional[XReplyTo] = Field(default=None, alias="replyTo")
    subject: StrictStr = Field(description="Override template subject")
    template: StrictStr = Field(description="Template identifier")
    __properties: ClassVar[List[str]] = ["from", "to", "replyTo", "subject", "template"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "from": XFrom.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": [XTo.from_dict(_item) for _item in obj["to"]] if obj.get("to") is not None else None,
            "replyTo": XReplyTo.from_dict(obj["replyTo"]) if obj.get("replyTo") is not None else None,
