...
class RestRTemplate(BaseModel):
    """
    RestRTemplate
    """ # noqa: E501
    id: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Unique template identifier with template_ prefix")
    name: Optional[StrictStr] = Field(default=None, description="Name of the template")
    subject: Optional[StrictStr] = Field(default=None, description="Email subject line (if applicable)")
    html_code: Optional[StrictStr] = Field(default=None, description="HTML content of the template", alias="htmlCode")
    template_code: Optional[StrictStr] = Field(default=None, description="Template code for visual editors (JSON structure)", alias="templateCode")
    type: Optional[StrictInt] = Field(default=None, description="Template type.  **Values:** - `0` - Email template - `1` - Other types ")
    thumbnail: Optional[StrictStr] = Field(default=None, description="URL to template thumbnail image")
    editor_type: Optional[StrictInt] = Field(default=None, description="Editor type used to create the template.  **Values:** - `0` - PlainText - `1` - DragDrop - `2` - SendxEditor ", alias="editorType")
    created: Optional[datetime] = Field(default=None, description="Template creation timestamp")
    updated: Optional[datetime] = Field(default=None, description="Template last update timestamp")
    __properties: ClassVar[List[str]] = ["id", "name", "subject", "htmlCode", "templateCode", "type", "thumbnail", "editorType", "created", "updated"]
    ...
    def id_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^template_[a-zA-Z0-9]{22}$", value):
            raise ValueError(r"must validate the regular expression /^template_[a-zA-Z0-9]{22}$/")
        return value
